	...
	$.esriPreview = function(elem, options) {
		var defaults = {},
			...
			plugin.ui = {
				...
				_url     : "",
				_root    : null,
				_panel   : $("<div style='position:relative' class='img-polaroid' ></div>"),
				_map     : $("<img style='position:absolute;top:4px;left:4px;display:none' />"),
				_bg      : $("<img style='position:absolute;top:4px;left:4px;display:none;' />"),
				_loading : $("<div style='position:absolute;top:70px;left:5px;text-align:center' ><i class='icon-spinner icon-spin icon-large'></i></div>"),
				_width   : 250,
				_height  : 250,
				...
				init : function(root, url, width, height, onLoad){
					...
					this._loading.height(this._height).width(this._width);
				...
				createImg : function(layer, info, fullExtent) {
					...
					if( this._sr.length > 0 ) {
						this._bg.attr('src', this._basemap+"/export?bbox="+this._bbox+"&format=png32&transparent=true&f=image&imageSR="+
								this._sr+"&bboxSR="+this._sr+"&size="+(this._width*2)+","+(this._height*2));
						this._bg.show();
					}
				...
				setError: function( errorMsg) {
					this._loading.hide();
					this._panel.html("<div style='"+this._height+"px'>"+errorMsg+"</div>");
